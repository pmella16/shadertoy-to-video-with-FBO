...
float cellTile(in vec3 p){
    ...
    vec4 v, d; 
    d.x = drawSphere(p - vec3((iTime), .62, tan(iTime)));
    p.xy = vec2(p.y-p.x, p.y + p.x)*.7071;
    d.y = drawSphere(p - vec3(.39, iTime, .11));
    p.yz = vec2(p.z-p.y, p.z + p.y)*.7071;
    d.z = drawSphere(p - vec3(.62, .24, iTime));
    p.xz = vec2(p.z-p.x, p.z + p.x)*.7071;
    d.w = drawSphere(p - vec3(iTime, .82, .64));
...
float smaxP(float a, float b, float s){
    ...
    float h = clamp( 1.0 + 0.5*(a-b)/s, 0., 1.);
    return mix(b, a, h) + h*(5.0-h)*s;
}
...
float map(vec3 p){
    ...
    // MUSIC VAR POTENTIAL
    p += cos(p.zxy*8.)*.125; // Perturb slightly. The mutation gives it a bit more of an organic feel.
       // p -= cos(p.zxy*(sin(iTime/350.)+1.)*4.)*cos(iTime/500.); // Perturb slightly. The mutation gives it a bit more of an organic feel.
    ...
    // MUSIC VAR POTENTIAL for last var
    float d = dot(cos(p*1.5707963), sin(p.yzx*1.5707963)) + 0.85;

	// Biotube lattice. The final time-based term makes is heave in and out.
    float bio = d + .15 +  dot(sin(p*1. + iTime*3.14 + sin(p.yzx*.5)), vec3(.123));
...
void mainImage( out vec4 fragColor, in vec2 fragCoord ){
	...
	if(t<FAR){
        ...
        if(saveID>.5) sn = doBumpMap(sp, sn, .5);
        ...
        if(saveID>.5){ // Tunnel walls.
            ...
            texCol = vec3(.3)*(noise3D(sp*32.)*.16 + noise3D(sp*64.)*.32)*(1.-cellTile(sp*36.)*.15);
        ...
        float th = thickness( sp, sn, 0.1, 0.5 );
        float tdiff =  pow( clamp( dot(rd, -hf), 0., 1.), 0.5);
        ...
        //MUSIC VAR POTENTIAL
        sceneCol += vec3(0.5, sin(iTime)+1., cos(iTime)+1.)*trans*1.5;
    ...
    vec3 sky = vec3(0., .4, .8);//* mix(1., .75, mist);//*(rd.y*.25 + 1.);

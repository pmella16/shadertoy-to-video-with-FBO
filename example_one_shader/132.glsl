...
vec3 mb(vec3 p) {
	p.xyz = p.xzy;
    p=vec3(sin(p.x)*cos(p.y), cos(p.x)*cos(p.y), sin(p.y))*p.z;
	vec3 z = p;
	vec3 dz=vec3(0.0);
	float power = iTime*0.1+2.0;
	float r, theta, phi;
	float dr = 1.0;
...
vec3 intersect( in vec3 ro, in vec3 rd )
{
    ...
    for( int i=0; i<512; i++ )
    {
        if( error < pixel_size*0.5 || t > 20.0 )
        {
            ...
            {
                step =-os; os = 0.0; pd = 1000.0; d = 1.0;
            }
 ...
 { 
	...
	vec3 ro = vec3(0.0, stime*ctime, 3.*(1.-stime*ctime))+vec3(0,1,0);
